...
void botonElegirMapaCB(int valor)
{
  ...
  panelSelecMapa->add_radiobutton_to_group(group, "Nivel 2: Con aldeanos y lobos                        ");
  panelSelecMapa->add_radiobutton_to_group(group, "Nivel 3: Sensor Terreno estropeado y Sin norte       ");
...
void lanzar_motor_grafico(int argc, char **argv)
{
  ...
  // Main Window
  ventanaPrincipal = glutCreateWindow("Practica 1: Agentes Reactivos. Curso 23/24");
...
void lanzar_motor_grafico_verOnline(int argc, char **argv, EnLinea &argumentos)
{
  ...
  // Main Window
  ventanaPrincipal = glutCreateWindow("Practica 1: Agentes Reactivos (23/24). Version BAJO PARAMETROS");
  // Main Window callback function
  glutReshapeFunc(reshape);
  glutDisplayFunc(display_ventPrincipal);
